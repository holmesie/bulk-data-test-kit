The Bulk Data Submit Recipient test suite validates the conformance of a recipient system
...
These tests verify the behavior of a data recipient by simulating a data
provider. The tests perform authorization, make a data submission, poll for the
submission status, and wait for the recipient to retrieve the data.

First, the recipient will likely need to register Inferno as a client. The
...
if none is provided. The FHIR endpoint of the recipient and an ID to use for the
